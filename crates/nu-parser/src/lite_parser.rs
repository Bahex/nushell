...
pub fn lite_parse(tokens: &[Token]) -> (LiteBlock, Option<ParseError>) {
    ...
    for (idx, token) in tokens.iter().enumerate() {
        match mode {
            ...
            Mode::Normal => {
                match file_redirection.take() {
                    Some((source, append, span)) => match &token.contents {
                    ...
                    None => {
                        match &token.contents {
                            ...
                            TokenContents::OutGreaterThan => {
                                ...
                                file_redirection =
                                    Some((RedirectionSource::Stdout, false, token.span));
                            }
                            TokenContents::OutGreaterGreaterThan => {
                                error = error.or(command.check_accepts_redirection(token.span));
                                file_redirection =
                                    Some((RedirectionSource::Stdout, true, token.span));
                            }
                            TokenContents::ErrGreaterThan => {
                                error = error.or(command.check_accepts_redirection(token.span));
                                file_redirection =
                                    Some((RedirectionSource::Stderr, false, token.span));
                            }
                            TokenContents::ErrGreaterGreaterThan => {
                                error = error.or(command.check_accepts_redirection(token.span));
                                file_redirection =
                                    Some((RedirectionSource::Stderr, true, token.span));
                            }
                            TokenContents::OutErrGreaterThan => {
                                error = error.or(command.check_accepts_redirection(token.span));
                                file_redirection =
